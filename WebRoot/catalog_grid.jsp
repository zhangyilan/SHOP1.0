			  ...
			  <c:forEach var="data" items="${sessionScope.categoryProducts}">
				  <c:choose>
					  <c:when test="${secondId == data.id}">
						  <li class="curent" ><a href="product_thirdCategoryAndProduct?id=${data.id}">${data.name}</a></li>
					  </c:when>
					  <c:otherwise>
    ...
    <div class="container_12">
       ...
       <div id="content" class="grid_12">
	      ...
	      <div class="grid_product grid_12">
			  <c:forEach var="data" items="${sessionScope.pageUtil.pageData}" varStatus="index">
				  <div class="grid_3 product" style="float: left;margin: 5px;" >
						  ...
						  <%--//${data.imgs[0].url}--%>
							  <c:forEach var="d" items="${data.imgs}" varStatus="index">
								  <c:if test="${index.index == 0}">
									  <img	 src="${pageContext.request.contextPath}/upload/product/${d.url}" alt="" title="" />
								  </c:if>
							  </c:forEach>
						  </a>
					  </div><!-- .prev -->
					  ...
					  <div class="cart">
						  ...
						  <a href="javascript:void(0)" onclick="subCart('${data.id}','${data.barginPrice}')" class="bay"></a>
					  </div><!-- .cart -->
				  </div><!-- .grid_3 -->
			  </c:forEach>
			  <script>
		     ...
		     <ul>
				 <c:choose>
					 <c:when test="${pageUtil.currentPage - 1 > 0}">
						 <li class="prev"><a href="product_nextPage?id=${secondId}&index=${pageUtil.currentPage -1}">&#8592;</a></li>
					 </c:when>
					 <c:otherwise>
					...
					<li class="curent"><a href="product_nextPage?id=${secondId}&index=${i}">${pageUtil.currentPage}</a></li>
					 <li ><a href="product_nextPage?id=${secondId}&index=${pageUtil.totalPage}">>></a></li>
				 <c:choose>
					 <c:when test="${pageUtil.currentPage != pageUtil.totalPage}">
						 <li class="next"><a href="product_nextPage?id=${secondId}&index=${pageUtil.currentPage+1}">&#8594;</a></li>
					 </c:when>
					 <c:otherwise>
						 <li class="prev"><a href="javascript:void(0);">&#8594;</a></li>
					 </c:otherwise>
